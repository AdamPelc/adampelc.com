...
class queue_lock_free_test_t : public ::testing::Test {};

using test_types_t = ::testing::Types<int, test_class_no_default_ctor_t>;
TYPED_TEST_SUITE(queue_lock_free_test_t, test_types_t);

TYPED_TEST(queue_lock_free_test_t, single_enqueue_dequeue) {
    spsc_queue::queue_lock_free_t<TypeParam, 2> queue;
...
TYPED_TEST(queue_lock_free_test_t, single_enqueue_single_discard) {
    spsc_queue::queue_lock_free_t<TypeParam, 2> queue;
    auto expected_element = TypeParam(0xDEAD);
...
TYPED_TEST(queue_lock_free_test_t, single_enqueue_dual_discard) {
    spsc_queue::queue_lock_free_t<TypeParam, 2> queue;
    auto expected_element = TypeParam(0xDEAD);
...
TYPED_TEST(queue_lock_free_test_t, dequeue_empty) {
    spsc_queue::queue_lock_free_t<TypeParam, 2> queue;
...
TYPED_TEST(queue_lock_free_test_t, discard_empty) {
    spsc_queue::queue_lock_free_t<TypeParam, 2> queue;
...
TYPED_TEST(queue_lock_free_test_t, muliple_enqueue_all_dequeue) {
    spsc_queue::queue_lock_free_t<TypeParam, 4> queue;
...
TYPED_TEST(queue_lock_free_test_t, muliple_enqueue_all_discard) {
    spsc_queue::queue_lock_free_t<TypeParam, 4> queue;
...
TYPED_TEST(queue_lock_free_test_t, multiple_enqueue_all_dequeue_oversize) {
    spsc_queue::queue_lock_free_t<TypeParam, 2> queue;
...
TYPED_TEST(queue_lock_free_test_t, multiple_enqueue_all_discard_oversize) {
    spsc_queue::queue_lock_free_t<TypeParam, 4> queue{};
...
TYPED_TEST(queue_lock_free_test_t, wraparound_behavior_size_one) {
    spsc_queue::queue_lock_free_t<TypeParam, 2> queue;
...
TYPED_TEST(queue_lock_free_test_t, mixed_enqueue_dequeue_operations) {
    spsc_queue::queue_lock_free_t<TypeParam, 4> queue;
...
TYPED_TEST(queue_lock_free_test_t, partial_fill_and_drain_cycles) {
    spsc_queue::queue_lock_free_t<TypeParam, 4> queue;
...
TYPED_TEST(queue_lock_free_test_t, exact_capacity_boundary) {
    spsc_queue::queue_lock_free_t<TypeParam, 4> queue{};
...
TEST(queue_lock_free_move_only_test_t, move_only_enqueue_dequeue) {
    spsc_queue::queue_lock_free_t<move_only_t, 2> queue{};
...
TEST(queue_lock_free_move_only_test_t, move_only_overwrite_behavior) {
    spsc_queue::queue_lock_free_t<move_only_t, 2> queue{};
...
TYPED_TEST(queue_lock_free_test_t, dequeue_failure_preserves_output_parameter) {
    spsc_queue::queue_lock_free_t<TypeParam, 2> queue;
...
TYPED_TEST(queue_lock_free_test_t, queue_state_consistency_after_operations) {
    spsc_queue::queue_lock_free_t<TypeParam, 4> queue;
...
TYPED_TEST(queue_lock_free_test_t, alternating_discard_dequeue_consistency) {
    spsc_queue::queue_lock_free_t<TypeParam, 4> queue;
...
TEST(queue_lock_free_performance_test_t, large_object_operations) {
    spsc_queue::queue_lock_free_t<large_object_t, 4> queue;
...
TEST(queue_lock_free_string_test_t, string_fifo_ordering) {
    spsc_queue::queue_lock_free_t<std::string, 8> queue;
...
TEST(queue_lock_free_string_test_t, string_overwrite_behavior) {
    spsc_queue::queue_lock_free_t<std::string, 4> queue{};
...
TEST(queue_lock_free_large_capacity_test_t, large_queue_behavior) {
    constexpr std::size_t large_size = 1024;
    spsc_queue::queue_lock_free_t<int, large_size> queue;
...
TEST(queue_lock_free_multithread_test_t, concurent_enqueue_dequeue) {
    ...
    std::jthread consumer_thread([&latch, &queue]() {
        latch.arrive_and_wait();
...
TEST(queue_lock_free_multithread_test_t, concurent_enqueue_discard) {
    constexpr auto number_of_threads = 2;
    std::latch latch(number_of_threads);
    spsc_queue::queue_lock_free_t<int, 1024> queue;
